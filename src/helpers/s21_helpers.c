...
/**
 * @brief Конвертация целого числа в стандартный десятичный формат
 *
 * Функция сначала конвертирует целое число в большой десятичный формат, а затем
 * конвертирует его в стандартный десятичный формат.
 *
...
s21_decimal s21_convert_int_to_std(unsigned long int number) {
  ...
  s21_big_decimal alt_decimal;
  alt_decimal = s21_convert_int_to_big(number);
  decimal = s21_to_std_decimal(alt_decimal);
  return decimal;
}

/**
 * @brief Конвертация целого числа в большой десятичный формат
 *
 * Функция конвертирует целое число в число типа s21_big_decimal.
 *
 * @param number Целое число для конвертации
 * @return Число типа s21_big_decimal
 */
...
/**
 * @brief Сдвигает число влево на заданное количество позиций.
 * @param d Десятичное число для сдвига.
 * @param count Количество позиций для сдвига.
 * @return Результат сдвига.
 */
s21_decimal s21_left_shift(s21_decimal d, int count) {
  s21_decimal result;
  s21_init_decimal(&result);
  for (int i = 0; i < 96; i++) {
    if (s21_get_bit(d, i)) s21_set_bit(&result, i, 1);
  }
  while (count) {
    for (int i = 94; i >= 0; i--) {
      s21_set_bit(&result, i + 1, s21_get_bit(result, i));
    }
...
/**
 * @brief Определяет позицию самого левого ненулевого бита в числе.
 * @param d Десятичное число.
 * @return Позиция самого левого ненулевого бита.
 */

int s21_last_bit(s21_decimal d) {
  int pos = -1;
  while (pos == -1) {
    for (int i = 95; i >= 0; i--) {
      if (s21_get_bit(d, i)) {
...
/**
 * @brief Умножает десятичное число на 10.
 * @param value_1 Десятичное число, которое нужно умножить.
 * @param result Указатель на число, в которое будет записан результат.
 * @return Статус выполнения операции.
 */
int s21_mul_by_ten(s21_decimal value_1, s21_decimal *result) {
  arithmetic_operation_result flag;
  flag = OK;
  s21_decimal ten;
  ...
  for (int i = 0; i < 96; i++) {
    if (s21_get_bit(value_1, i)) {
      s21_decimal sum = s21_left_shift(ten, i);
      s21_add(*result, sum, result);
    }
  }
